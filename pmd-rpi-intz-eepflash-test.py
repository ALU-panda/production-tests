# This is a modified version of the production script for the PMD-RPI-INTZ evaluation board from Analog Devices, Inc.
# Only use this script to check if the eepflash utility is working. NEW BOARDS SHOULD STILL BE TESTED USING THE FULL SCRIPT!
# The EEPROM utilities published in https://github.com/raspberrypi/hats/tree/master/eepromutils must be installed to
# successfully execute this script. Do not edit any part of the code without consent from the Systems Development Group.
# - Allan Uy (18 Jan 2022)
...
def flash_eeprom():
    ...
    call([eepromutils_path + '/eepflash.sh', '-w', '-y', '-f=' + eepfile_path + '/blank.eep', '-t=24c32'])
    sleep(1)

    print('\nUploading *.eep file to EEPROM:')
    call([eepromutils_path + '/eepflash.sh', '-w', '-y', '-f=' + eepfile_path + '/pmd_rpi_intz.eep', '-t=24c32'])
    sleep(1)
...
def verify_eeprom():
    ...
    call([eepromutils_path + '/eepflash.sh', '-r', '-y', '-f', eepromdump_path + '/dump.eep', '-t=24c32'])
    call([eepromutils_path + '/eepdump', eepromdump_path + '/dump.eep', eepromdump_path + '/dump.txt'])
